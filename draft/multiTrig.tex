...
For any~$w, w' \in \{u,v\}^k$ which only differ at position~$i \in [k]$, the $k$-triangulations~$T_w$ and~$T_w'$ are the only two $k$-triangulations~$\surface$ which contain~${T_w \ssm \{a_w^i\} = T_{w'} \ssm \{a_{w'}^i\}}$. In other words, any diagonal~$a_w^i$ in the $k$-triangulation~$T_w$ can be uniquely flipped.
\end{proposition}

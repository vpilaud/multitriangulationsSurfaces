...
Namely, it corresponds to an intersection graph with at most $\ell$ edges, with no $(\ell+2)$-clique, or with maximal degree~$\ell$.
...
A \defn{$k$-triangulation} of a convex $n$-gon is a maximal set of diagonals with no subset of $k+1$ mutually crossing diagonals.
...
\item The set of $k$-triangulations of the $n$-gon is enumerated by determinant of Catalan numbers~\cite{Jonsson}. It is in bijection with families of $k$ mutually non-crossing Dyck paths~\cite{SerranoStump}.
\end{enumerate}

The objective of this note is to introduce the good notion of $k$-triangulation on a surface such that properties (i) to (iii) above hold.

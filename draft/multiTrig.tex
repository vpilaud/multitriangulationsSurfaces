...
This operation, illustrated in \cref{fig:cutAlongEdge}, is easier to see on the disk, and can then be extended to surfaces using the universal cover.
%
\begin{figure}[t]
	\capstart
	\centerline{\includegraphics[scale=.5]{2triangCylinderCut}}
	\caption{Cutting a $2$-triangulation of a cylinder along an edge.}
	\label{fig:cutAlongEdge}
\end{figure}
%
...
Indeed, since
\begin{itemize}
\item each $k$-relevant edge belongs to $2$ $k$-stars while each $k$-boundary edge belongs to one $k$-star,

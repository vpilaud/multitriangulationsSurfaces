...
Since there is only one star, all $k$-border edges have to be part of the star. We denote $L$ the maximal length of an edge.
...
Let $l$ be a length such that $k<l<L$. Since the star is represented by a (connected) excursion, there exists a vertex $i$ such that $(i,l)$ has symbol $\circ$. By consequence, $n_\circ\geq L-k-1$.

By unicity of the red $\times$, the symbol coming just after the red $\times$ has to have length strictly bigger. By consequence $h<L$.
